  ...
  if (Serial.available() > 0) {  //если есть доступные данные
        ...
        if (data1 != 0)
        {
          ...
          if (data == 21)
            {
            adc_data=ads1220.Read_WaitForData();
            Serial.print(millis()); Serial.print(", "); Serial.println(adc_data);// Serial.print(pinD3count); Serial.print(", "); Serial.print(pinA0state);Serial.print(", "); Serial.println(pinA2state);
            pinA0state=digitalRead(A0);
            pinA2state=digitalRead(A2);
            autochangedir();
            }
